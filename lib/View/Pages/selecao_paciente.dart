...
class _SelecaoPacienteState extends State<SelecaoPaciente> {
  ...
  Future<void> _carregarPacientes() async {
    ...
    try {
      ...
      final pacientes = await VinculoService.listarMeusPacientesVinculados(token ?? '');
      if (!mounted) return;
      setState(() {
      ...
      if (!mounted) return;
      setState(() => _erro = "Não foi possível carregar os pacientes vinculados.");
    } finally {
  ...
  List<PacienteResumoModel> _aplicarFiltros({String nome = '', int? nivel}) {
    return _pacientes.where((paciente) {
      final correspondeNome = paciente.nome.toLowerCase().contains(nome.toLowerCase());
      final correspondeNivel = nivel == null || _nivel(paciente.grauAutismo) == nivel;
  ...
  int _nivel(GrauAutismo grauAutismo) {
    switch (grauAutismo) {
      case GrauAutismo.nivel1: return 1;
      case GrauAutismo.nivel2: return 2;
      case GrauAutismo.nivel3: return 3;
    }
  }
        ...
        children: [
          Padding(
            ...
            child: TextFormField(
              ...
              decoration: InputDecoration(
                ...
                prefixIcon: Icon(Icons.search, color: cores.onSurface.withOpacity(0.5)),
                suffixIcon: meuController.text.isEmpty ? null : IconButton(
          ...
          SingleChildScrollView(
            ...
            child: Row(
              children: [
                ...
                BotaoPersonalizadoFiltro(onPressed: () => filtrarListaPorNivel(null), texto: 'Todos', corFundo: nivelSelecionado == null ? cores.primary : cores.onPrimary.withOpacity(0.3), corLetra: nivelSelecionado == null ? cores.onPrimary : cores.onSurface.withOpacity(0.5)),
                const SizedBox(width: 10),
                BotaoPersonalizadoFiltro(onPressed: () => filtrarListaPorNivel(1), texto: 'Nível 1', corFundo: nivelSelecionado == 1 ? cores.primary : cores.onPrimary.withOpacity(0.3), corLetra: nivelSelecionado == 1 ? cores.onPrimary : cores.onSurface.withOpacity(0.5)),
                const SizedBox(width: 10),
                BotaoPersonalizadoFiltro(onPressed: () => filtrarListaPorNivel(2), texto: 'Nível 2', corFundo: nivelSelecionado == 2 ? cores.primary : cores.onPrimary.withOpacity(0.3), corLetra: nivelSelecionado == 2 ? cores.onPrimary : cores.onSurface.withOpacity(0.5)),
                const SizedBox(width: 10),
                BotaoPersonalizadoFiltro(onPressed: () => filtrarListaPorNivel(3), texto: 'Nível 3', corFundo: nivelSelecionado == 3 ? cores.primary : cores.onPrimary.withOpacity(0.3), corLetra: nivelSelecionado == 3 ? cores.onPrimary : cores.onSurface.withOpacity(0.5)),
                const SizedBox(width: 10),
              ],
            ),
          ),
          Padding(
            ...
            child: ElevatedButton(
              onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (context) => const CadastroPaciente())),
          ...
          Expanded(
            child: _carregando
                ...
                : _erro != null
                    ? Center(child: Padding(padding: const EdgeInsets.all(24.0), child: Text(_erro!, style: TextStyle(color: cores.error))))
                    : pacientesFiltrados.isEmpty
                        ? Center(child: Text("Nenhum paciente vinculado encontrado.", style: TextStyle(color: cores.onSurface.withOpacity(0.6))))
                        : ListView.separated(
